...
zed = sl.Camera()
window = Tk()
window.title("Tracking GUI")
window.geometry("1000x250")
window.resizable(0,0)
window.configure(background="black")
position = Label(window, font=('fixedsys', 20, 'bold'), fg='white', bg='black')
position.pack(fill=NONE, expand=1, side=TOP)
orientation = Label(window, font=('fixedsys', 20, 'bold'), fg='white', bg='black')
orientation.pack(fill=NONE, side=BOTTOM)
...
def track():
    if zed.grab(runtime_parameters) == sl.ERROR_CODE.SUCCESS:
        ...
        '''
        position.config(text=transform)
        orientation.config(text=rotation)
        position.after(100, track)
